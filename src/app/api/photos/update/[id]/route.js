 ...
 *
 * Logging: ver convención en /api/photos/create/route.js (debug/info/warn/error gateados por
 * NODE_ENV + API_DEBUG).
 *
...
export async function PUT(request, { params }) {
  try {
    ...
    log.debug(`Parámetros recibidos:`, resolvedParams)
    log.debug(`ID extraído:`, id)

    if (!id) {
      log.error(`❌ ID no encontrado en params. Params recibidos:`, resolvedParams)
    ...
    log.debug(`=== FORMDATA ENTRANTE (ID: ${id}) ===`)
    ...
    if (imageKeys.length === 0) {
      log.debug('ℹ️ No hay imágenes en el FormData (esto es válido en UPDATE)')
    } else {
      log.debug(`ℹ️ Imágenes encontradas: ${imageKeys.map(k => k.key).join(', ')}`)
    }
    ...
    if (backendResponse.status === 400) {
      ...
      log.error('Verificar:')
      log.error('  - ¿El ID del vehículo es válido?')
      log.error('  - ¿Los campos enviados son correctos?')
